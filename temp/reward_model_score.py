...
def construct_ensemble(self):
    ...
    for i in range(self.de):
        if not self.image_reward:
            ...
            model = nn.Sequential(
                nn.Linear(768, self.reward_model_H),
                nn.LeakyReLU(),
                nn.Linear(self.reward_model_H, 1)
            ).float().to(device)
...
class RewardModelScore:
    ...
    def r_hat_member(self, x, member=-1):
        if self.image_reward:
            # 確保輸入符合 DeiT 的預期 (B, C, H, W)
            if not isinstance(x, torch.Tensor):
                x = torch.from_numpy(x).float().to(device)
            
            # 使用凍結的 DeiT 提取視覺特徵
            with torch.no_grad():
                # 注意：這裡可能需要根據 image_processor 的需求將圖片 resize 到 224x224
                # 為求效能，可以直接使用 torch.nn.functional.interpolate 進行 resize
                x_resized = F.interpolate(x, size=(224, 224), mode='bilinear', align_corners=False)
                outputs = self.feature_extractor(pixel_values=x_resized)
                # 取用 [CLS] token 的特徵作為整張圖片的 representation
                features = outputs.last_hidden_state[:, 0, :] 
                
            return self.ensemble[member](features)
        else:
