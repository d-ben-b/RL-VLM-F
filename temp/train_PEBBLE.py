    ...
    def evaluate(self, save_additional=False):
        ...
        for episode in range(self.cfg.num_eval_episodes):
            ...
            save_gif_path = os.path.join(
                save_gif_dir,
                "step{:07}_episode{:02}_{}.gif".format(
                    self.step, episode, round(true_episode_reward, 2)
                ),
            )
            ...
            if save_additional:
                ...
                for i, image in enumerate(images):
                    save_image_path = os.path.join(
                        save_image_dir,
                        "step{:07}_episode{:02}_{}.png".format(self.step, episode, i),
                    )
                ...
                with open(
                    os.path.join(
                        save_reward_path,
                        "step{:07}_episode{:02}.pkl".format(self.step, episode),
                    ),
                    "wb",
                ) as f:
                    pkl.dump(rewards, f)
        ...
        while self.step < self.cfg.num_train_steps:
            if done:
                if self.step > 0:
                    ...
                    self.logger.log(
                        "train/reward_learning_acc", reward_learning_acc, self.step
                    )
            ...
            if self.step == (self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
                ...
                # update schedule
                if (
                    self.reward == "learn_from_preference"
                    or self.reward == "learn_from_score"
                ):
            ...
            if (
                self.cfg.vlm_label
                or self.reward
                in ["blip2_image_text_matching", "clip_image_text_matching"]
                or (
                    self.cfg.image_reward
                    and self.reward not in ["gt_task_reward", "sparse_task_reward"]
                )
            ):
                if "metaworld" in self.cfg.env:
                    rgb_image = self.env.render()
                ...
                if (
                    self.cfg.image_reward
                    and "Water" not in self.cfg.env
                    and "Rope" not in self.cfg.env
                ):
            ...
            if (
                ...
                reward_hat = (
                    blip2_image_text_matching(query_image, query_prompt) * 2 - 1
                )  # actually we should scale it [-1, 1] since tanh is used in the reward model
                ...
                query_prompt = clip_env_prompts[self.cfg.env]
                reward_hat = (
                    clip_image_text_matching(query_image, query_prompt) * 2 - 1
                )  # actually we should scale it [-1, 1] since tanh is used in the reward model
                if self.cfg.flip_vlm_label:
                    reward_hat = -reward_hat
            ...
            if "softgym" not in self.cfg.env:
                done_no_max = (
                    0 if episode_step + 1 == self.env._max_episode_steps else done
                )
            else:
                done_no_max = done
            ...
            if self.cfg.image_reward and self.reward not in [
                "gt_task_reward",
                "sparse_task_reward",
            ]:
