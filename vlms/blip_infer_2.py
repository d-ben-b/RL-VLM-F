...
if load_model_and_preprocess is not None:
    model, vis_processors, text_processors = load_model_and_preprocess(
        "blip2_image_text_matching", "pretrain_vitL", device=device, is_eval=True
    )
else:
    model = vis_processors = text_processors = None


def blip2_infer_image_text_matching(
    rgb1, rgb2, text, use_prob=False, return_scores=False
):
